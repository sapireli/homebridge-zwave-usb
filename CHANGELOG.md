...
- **Global Health Monitoring (Unreachable Nodes):** Added universal support for HomeKit's `StatusFault` characteristic across all functional services. If the Z-Wave JS controller marks a device as "Dead" or it fails to complete its initial interview (due to power loss, distance, etc.), it will instantly transition to "No Response" in the Apple Home app.
