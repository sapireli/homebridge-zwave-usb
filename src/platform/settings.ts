export const PLUGIN_NAME = 'homebridge-zwave-usb';
export const PLATFORM_NAME = 'ZWaveUSB';

// Custom UUIDs - Version 6
// Definitive reset to clear metadata cache
export const MANAGER_SERVICE_UUID = '00000001-0000-1000-8000-0026BB765296';
export const STATUS_CHAR_UUID = '00000002-0000-1000-8000-0026BB765296';
export const PIN_CHAR_UUID = '00000003-0000-1000-8000-0026BB765296';

// HAP Constants
export enum HAPFormat {
  STRING = 'string',
  BOOL = 'bool',
  INT = 'int',
  FLOAT = 'float',
  UINT8 = 'uint8',
  UINT16 = 'uint16',
  UINT32 = 'uint32',
  UINT64 = 'uint64',
  DATA = 'data',
  TLV8 = 'tlv8',
}

export enum HAPPerm {
  PAIRED_READ = 'pr',
  PAIRED_WRITE = 'pw',
  NOTIFY = 'ev',
  HIDDEN = 'hd',
}

// Obsolete UUIDs for cleanup
export const OBSOLETE_MANAGER_UUIDS = [
    '00000001-0000-1000-8000-0026BB765291',
    '00000001-0000-1000-8000-0026BB765292',
    '00000001-0000-1000-8000-0026BB765293',
    '00000001-0000-1000-8000-0026BB765294',
    '00000001-0000-1000-8000-0026BB765295', // v5
    '9f8e7d6c-5b4a-3f2e-1d0c-9b8a7f6e5d4c'
];

export const OBSOLETE_CHAR_UUIDS = [
    '00000002-0000-1000-8000-0026BB765291',
    '00000003-0000-1000-8000-0026BB765291',
    '00000002-0000-1000-8000-0026BB765292',
    '00000003-0000-1000-8000-0026BB765292',
    '00000002-0000-1000-8000-0026BB765293',
    '00000003-0000-1000-8000-0026BB765293',
    '00000002-0000-1000-8000-0026BB765294',
    '00000003-0000-1000-8000-0026BB765294',
    '00000002-0000-1000-8000-0026BB765295',
    '00000003-0000-1000-8000-0026BB765295',
    '7f8e9d0a-1b2c-4d3e-8f9a-0b1c2d3e4f5a',
    '8a9b0c1d-2e3f-4a5b-9c6d-7e8f9a0b1c2d',
    'E863F108-079E-48FF-8F25-9C2566232931',
    'E863F109-079E-48FF-8F25-9C2566232931'
];
