...
export class ContactSensorFeature extends BaseFeature {
  ...
  init(): void {
    ...
    this.service
      ...
      .on('get', this.handleGetContactSensorState.bind(this));
  }
  ...
  private getSensorValue(): number {
    // 1. Check Notification CC (Access Control)
    if (this.node.supportsCC(113)) {
        ...
        if (typeof val === 'number') {
            ...
            if (val === 23) return this.platform.Characteristic.ContactSensorState.CONTACT_DETECTED; // Closed
        }
    }
  ...
  private async handleGetContactSensorState(callback: CharacteristicGetCallback) {
    callback(null, this.getSensorValue());
  }
}
